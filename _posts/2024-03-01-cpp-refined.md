...
Brief Introduction and History of C++?

C++ is a programming language developed by Bjarne Stroustrup in the early 1980s at Bell Labs.  

C++ is often used to develop compilers and virtual machines because of its strong performance.  
...
In LLDB, the command `(lldb) image lookup --address 0x400610` is used to check which source code location corresponds to the memory address `0x400610`.  
...
### codes snnipets explain

1. `char* buffer = new char[8];`.

 This line of code will allocate a 8 bytes space, and the pointer is the address of the space beginning.
...
Every time a function is called, a new frame is pushed onto the call stack. And the frame contains:
     - Function parameters
     - Local variables
...
keyword `volatile`:
    ...
    - Informs the compiler not to optimize away a variable, especially when it maps to hardware registers.  
    - Ensures the CPU always reads the latest value from memory.  

keyword `static`:

    - Inside a function: the variable keeps its value between function calls.  
    - Outside functions: limits the variable’s visibility to the current file only.  

keyword `extern`:

    - Instructs the compiler to use C linkage for the specified code.  
    - Example: C++ supports function overloading, but C does not. `extern "C"` ensures compatibility.  


keyword `auto`:

    - In C++, `auto` is a **type deduction keyword** that allows the compiler to automatically infer the variable type from its initializer expression.  




## Links to something about cpp
